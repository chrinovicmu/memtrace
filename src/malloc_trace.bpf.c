...
#include <cstddef>
#include <sys/types.h>
#include "include/malloc_trace.h"
...
int BPF_KRETPROBE(on_malloc_exit, void *ptr)
{
    ...
    data.pid = pid; 
    data.tgid = pid; 
    data.uid = (u32)(bpf_get_current_uod_gid() & 0xFFFFFFFF); 
    data.ts = bpf_ktime_get_ns(); 
    data.cpu = bpf_get_smp_processor_id(); 
    data.bytes_alloc = (u64)(*sizep); 
    data.bytes_freed = 0; 
    
    bpf_get_current_comm(&data.command, sizeof(COMMAND_LEN)); 
